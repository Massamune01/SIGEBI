...
{
    public class CredencialesFacade : ICredencialesFacade
    {
        ...
        private readonly ILogger<CredencialesFacade> _logger;

        public CredencialesFacade(
            ICredencialesRepository credencialesRepository,
            IValidatorBase<CredencialesGetModel> validator,
            ILogger<CredencialesFacade> logger)
        {
        ...
        public async Task<ServiceResult> CreateCredencialesAsync(CredencialesCreateDto createCredencialesDto)
        {
            ...
            try
            {
                ...
                CredencialesGetModel credencialesGetModel = new CredencialesGetModel()
                {
                    ...
                    PasswordHash = createCredencialesDto.PasswordHash
        ...
        public async Task<ServiceResult> GetCredencialesByIdAsync(int id)
        {
            ...
            try
            {
                ...
                if (oCredenciales.Success)
                {
                    result.Success = true;
                    result.Message = "Retrieving Credenciales Succesfully.";
                    result.Data = oCredenciales.Data;
                    _logger.LogInformation("Credenciales with ID: {CredId} retrieved successfully.", id);
                }
                else
                {
                    result.Success = false;
                    result.Message = "Credenciales not found.";
                    _logger.LogWarning("Credenciales with ID: {CredId} not found.", id);
                }
            }
            catch (Exception ex)
            {
